...
// ============ Absence Chart with Filter ============
function renderAbsenceChart(moduleFilter="") {
  const ctx = document.getElementById("moduleAbsChart");
  if (!ctx) return;

  let labels = Object.keys(report.total_absences_by_module || {});
  let values = Object.values(report.total_absences_by_module || {});

  if (moduleFilter) {
    labels = labels.filter((m) => m === moduleFilter);
    values = values.filter((_, i) => labels[i] === moduleFilter);
  }

  if (window.absChartInstance) window.absChartInstance.destroy();

  window.absChartInstance = new Chart(ctx, {
    type: "bar",
    data: {
      labels: labels,
      datasets: [{
...
const absFilter = document.getElementById("absModuleFilter");
if (absFilter) {
  absFilter.addEventListener("change", (e) => {
    renderAbsenceChart(e.target.value);
  });
}
