(function () {
  ...
  // ===================== MODULES CHART (filters you already had) =====================
  ...
  // Only run if the heatmap section exists in the page
  const needIds = [
    "heatModule",
    "heatQual",
    "heatMode",
  ...
  const idToQualHM = {};
  (report.student_lookup || []).forEach((s) => {
    idToNameHM[s.id] = s.name || "";
    idToQualHM[s.id] = s.qual || "";
  });
  ...
  function buildHeatRows(mod, qual, sortKey = "total_desc") {
    ...
    Object.keys(byMod).forEach((sid) => {
      if (qual && (idToQualHM[sid] || "") !== qual) return;
  ...
  function renderHeatLegend(mode) {
    ...
    legend.innerHTML = `
      <span class="tiny muted">Legend:</span>
      <div style="height:10px;width:180px;border-radius:6px;border:1px solid rgba(255,255,255,.15);background:${bar};margin:0 6px;display:inline-block;vertical-align:middle"></div>
      <span class="tiny muted">${mode==="binary"?"0":"0%"}</span>
      <span class="tiny muted">→</span>
      <span class="tiny muted">${mode==="binary"?"1":"100%"}</span>
    `;
  }
  ...
  // Hook button and first render (auto-load matrix plugin)
